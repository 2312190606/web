package com.shliu.aichat.service.impl;

import com.shliu.aichat.model.ChatRoom;
import com.shliu.aichat.service.AiManage;
import com.shliu.aichat.service.ChatService;
...
public class ChatServiceImpl implements ChatService {
    ...
    public String doChat(long roomId, String userPrompt) {
        ...
        if(!chatHistories.containsKey(roomId)) {
            // 如果房间不存在，需要先创建
            if(userPrompt.equals("开始")) {
                messages = new ArrayList<>();
                messages.add(systemMessage);
                chatHistories.put(roomId, messages);
            } else {
                // 房间不存在且不是"开始"，返回错误提示
                throw new IllegalArgumentException("房间不存在，请先发送'开始'来创建新游戏");
            }
        } else {
            // 房间已存在，获取历史消息
